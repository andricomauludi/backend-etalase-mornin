...
func TotalCurrentMonth(c *gin.Context) {
	...
	if err := models.DB.Model(&Bill).
		...
		Where("timestamp BETWEEN ? AND ?", startOfMonth, endOfMonth).
		Scan(&total).
		Error; err != nil {
...
func TotalToday(c *gin.Context) {
	...
	if err := models.DB.Model(&Bill).
		Select("SUM(total) as total").
		Where("timestamp BETWEEN ? AND ?", startOfToday, endOfToday).
		Scan(&total).
		Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
...
func TotalPengeluaranCurrentMonth(c *gin.Context) {
	...
	if err := models.DB.Model(&pengeluaran).
		...
		Where("waktu_pengeluaran BETWEEN ? AND ?", startOfMonth, endOfMonth).
		Scan(&total).
		Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
...
func TotalPengeluaranToday(c *gin.Context) {
	...
	if err := models.DB.Model(&pengeluaran).
		Select("COALESCE(SUM(total_pengeluaran), 0) as total").
		Where("waktu_pengeluaran BETWEEN ? AND ?", startOfDay, endOfDay).
		Scan(&total).
		Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
